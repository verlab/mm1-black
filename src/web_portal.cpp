 ...
 *  - The portal is opt-in: it only starts when the user enables WiFi from
 *    the device WIFI tab, so the radio coexists with Bluetooth Classic only
...
bool start(const char* ssid, const char* password, const Callbacks& cb)
{
    ...
    WiFi.mode(WIFI_OFF);
    delay(80);
    /* AP-only mode after a clean transition — avoids "softAP stuck" after a prior OFF. */
    if (!WiFi.mode(WIFI_AP)) {
        g_cb = {};
        return false;
    }
    WiFi.setSleep(false);   /* avoid coexistence stalls when BT also runs */
